var title="Test";
    ...
    format = d3.time.format("%d-%m-%Y %H:%M");
    toolDate = d3.time.format("%d/%b/%y %H:%M");
    
    d3.json("datafiles/dummyData.json", function(error, data) {
        ...
        data.forEach(function(d)    {
                dates.push(parseDate(d.Tijd));
                values.push(d.ID);
                console.log(d.Tijd+"re");
                d.Tijd=parseDate(d.Tijd);
                d.value=d.ID;
            //console.log(d.Tijd.getFullYear()+"year");
                //d.year=d.Tijd.getFullYear();//extract the year from the data
        });
        ...
        var dataRects = cals.append("g")
            ...
            .append("rect")
            .attr("id",function(d) {
                return format(d.Tijd)+":"+d.value;
            })
            .attr("stroke","#ccc")
            .attr("width",cellSize)
            .attr("height",cellSize)
            .attr("x", function(d){return xOffset+calX+(d3.time.weekOfYear(d.Tijd) * cellSize);})
            .attr("y", function(d) { return calY+(d.Tijd.getDay() * cellSize); })
        ...
        dataRects.append("title")
            .text(function(d) { return toolDate(d.Tijd)+":\n"+d.value+units; });
